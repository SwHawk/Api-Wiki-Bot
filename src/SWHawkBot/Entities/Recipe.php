...
class Recipe {
	...
	/**
	 * Setter général pour ...OutputItemId, en fonction du type
	 * 
	 * @param integer $id
	 * @param string $type
    ...
    public function setOutputItemId($id, $type) {
        if(in_array($type,array(self::API_TYPE_AMULET, self::API_TYPE_EARRING, self::API_TYPE_RING))) {
			$this->setTrinketOutputItemId($id);
		}
		if(in_array($type,array(self::API_TYPE_BOOTS, self::API_TYPE_HELM,
		 self::API_TYPE_COAT, self::API_TYPE_GLOVES, self::API_TYPE_LEGGINGS,
		 self::API_TYPE_SHOULDERS, self::API_TYPE_HELMAQUATIC))) {
			$this->setArmorOutputItemId($id);
		}
		if(in_array($type,array(self::API_TYPE_AXE, self::API_TYPE_DAGGER,
		 self::API_TYPE_FOCUS, self::API_TYPE_GREATSWORD, self::API_TYPE_HAMMER,
		 self::API_TYPE_HARPOON, self::API_TYPE_LONGBOW, self::API_TYPE_MACE,
		 self::API_TYPE_PISTOL, self::API_TYPE_RIFLE, self::API_TYPE_SCEPTER,
		 self::API_TYPE_SHIELD, self::API_TYPE_SHORTBOW, self::API_TYPE_SPEARGUN,
		 self::API_TYPE_STAFF, self::API_TYPE_SWORD, self::API_TYPE_TORCH,
		 self::API_TYPE_TRIDENT, self::API_TYPE_WARHORN))) {
			$this->setWeaponOutputItemId($id);
		}
		
		if(in_array($type,array(self::API_TYPE_CONSUMABLE, self::API_TYPE_DESSERT,
		 self::API_TYPE_DYE, self::API_TYPE_FEAST, self::API_TYPE_MEAL,
		 self::API_TYPE_POTION, self::API_TYPE_SNACK, self::API_TYPE_SOUP))) {
			$this->setConsumabletOutputItemId($id);
		}
		
		if(in_array($type,array(self::API_TYPE_BAG))) {
			$this->setBagOutputItemId($id);
		}
		
		if(in_array($type,array(self::API_TYPE_COMPONENT, self::API_TYPE_INGREDIENT_COOKING,
		 self::API_TYPE_INSCRIPTION, self::API_TYPE_INSIGNIA, self::API_TYPE_REFINEMENT,
		 self::API_TYPE_REFINEMENT_ECTOPLASM, self::API_TYPE_REFINEMENT_OBSIDIAN))) {
			$this->setCraftingMaterialOutputItemId($id);
		}
		
		if(in_array($type,array(self::API_TYPE_UPGRADE_COMPONENT))) {
			$this->setUpgradeComponentOutputItemId($id);
		}
		
		if(in_array($type,array(self::API_TYPE_BULK))) {
			$this->setContainerOutputItemId($id);
		}
		
		if(in_array($type,array(self::API_TYPE_BACKPACK))) {
